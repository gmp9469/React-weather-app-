...
export const getEventDayDate = (day) => {
  ...
  const eventDayIndex = getEventDayIndex(day);
  let daysUntilEventDay = eventDayIndex - currentDayIndex;

  if (daysUntilEventDay < 0) {
    daysUntilEventDay += 7;
  }

  const eventDayDate = new Date(now);
  eventDayDate.setDate(now.getDate() + daysUntilEventDay);
